...
const { data, isLoading, isError, refetch } = useWorkoutPlansQuery({ page })
  ...
  <div class="space-y-6 p-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-muted-foreground">Browse the workout plans you've built.</p>
      </div>
      <RouterLink
        :to="{ name: 'workout-plan-new' }"
    ...
    <div v-if="isLoading" role="status" aria-label="Loading workout plans" class="space-y-2">
      <Skeleton v-for="n in 5" :key="n" class="h-10 w-full" />
    </div>

    <div v-else-if="isError" role="alert" class="space-y-2 text-sm text-destructive">
      <p>Something went wrong while loading workout plans.</p>
    ...
    >
      <p class="text-sm text-muted-foreground">No workout plans found.</p>
      <RouterLink
        :to="{ name: 'workout-plan-new' }"
        class="inline-flex h-10 items-center justify-center whitespace-nowrap rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground ring-offset-background transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
      >
    ...
    <template v-else-if="data">
      <Table>
        ...
        <TableBody>
          <TableRow v-for="workoutPlan in data.data" :key="workoutPlan.id">
            <TableCell class="font-medium">
              {{ workoutPlan.name }}
            </TableCell>
            <TableCell>
              <Badge variant="secondary">{{ workoutPlan.workout_sessions.length }}</Badge>
            </TableCell>
          </TableRow>
        </TableBody>
      </Table>
